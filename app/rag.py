...
from langchain_core.documents import Document as LangChainDocument  #https://stackoverflow.com/questions/76551067/how-to-create-a-langchain-doc-from-an-str
...
if not OPENROUTER_API_KEY:
    raise ValueError("OPENROUTER_API_KEY must be set in the environment variables (.env file).")
...
async def load_and_ingest_documents(files: List[UploadFile], html_file: UploadFile):
    ...
    if html_file:
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Error processing HTML file: {file.filename}")
...
async def generate_test_cases(query: str) -> List[TestCase]:
    context_texts = await retrieve_context(query)
    context_str = "\n\n".join(context_texts)
    prompt = f"""
    ...
    Use the following context to understand the features and rules:
    <context>
    {context_str}
    </context>
    ...
    - "Grounded_In": The name(s) of the document(s) from the context that justify this test case (e.g., "product_specs.md", "ui_ux_guide.txt").
